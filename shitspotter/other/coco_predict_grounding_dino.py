...
# # --- Replace for-loop with parallel DataLoader processing ---
# dataset = KwcocoImageDataset(
#     coco_images=list(dst_dset.images().coco_images),
#     processor=processor,
#     text_labels=text_labels,
# )

# loader = torch_data.DataLoader(dataset, num_workers=4, batch_size=1, collate_fn=collate_fn)

# for batch in ub.ProgIter(loader, desc='predict images', total=len(dataset)):
#     item = batch[0]  # batch_size=1, so just one item
#     inputs = item['inputs'].to(device)
#     pil_img_size = item['pil_img_size']
#     img_id = item['img_id']
#     coco_img = item['coco_img']
...
#     new_anns = list(dets.to_coco(image_id=img_id))
#     for ann in new_anns:
#         ann['category_id'] = dst_dset.ensure_category(ann['category_name'])
