...
# v8 is a full retrain on the new, larger dataset split
# (train_imgs10671_b277c63d / vali_imgs1258_577e331c / test_imgs121_d39956b1).
#
# Design rationale:
# - The new splits are roughly 2x larger in train and vali, so we retrain both
#   the SAM2 segmenter and the DEIMv2 detector from scratch on this data.
#   Reusing the v5 SAM2 checkpoint would be unfair to the new data volume.
# - Detector LR schedule follows v7 (V8_GLOBAL_LR_SCALE=0.8 relative to the
#   default batch-32 LR), which was the best-performing LR choice so far.
# - Checkpoint selection continues to use combined-raw-vali AP as the primary
#   key (validated in v6/v7), so test is only touched once at the very end.
...
V8_ROOT="$EXPT_DPATH/foundation_detseg_v3/v8"
DETECTOR_WORKDIR="$V8_ROOT/train_detector_deimv2_m"
SAM2_WORKDIR="$V8_ROOT/train_segmenter_sam2_1_hiera_base_plus"
CHECKPOINT_SELECT_DPATH="$V8_ROOT/checkpoint_select"
...
SELECTED_MANIFEST_FPATH="$V8_ROOT/selected_detector_checkpoint.yaml"

PACKAGE_DPATH="$REPO_DPATH/experiments/foundation_detseg_v3/packages"
FINAL_TUNED_PACKAGE_FPATH="$PACKAGE_DPATH/v8_deimv2_m_sam2_1_hiera_base_plus_tuned.yaml"
...
# Detector training - carry forward v7's best LR setting (global scale 0.8)
...
V8_GLOBAL_LR_SCALE="${V8_GLOBAL_LR_SCALE:-0.8}"
V8_BACKBONE_LR_RATIO="${V8_BACKBONE_LR_RATIO:-1.0}"

# SAM2 training - same hyperparameters as v5
...
CANDIDATES=(${V8_CANDIDATES:-checkpoint0019 checkpoint0024 checkpoint0029 checkpoint0034 checkpoint0039 checkpoint0044 checkpoint0049 checkpoint0054 checkpoint0059 best_stg1 last})
...
# Build detector LR config
if [ -z "${DEIMV2_CONFIG_OVERRIDES:-}" ]; then
    read -r BACKBONE_LR MAIN_LR <<EOF
$("$PYTHON_BIN" - <<PY
train_batch = int("$TRAIN_BATCH_SIZE")
base_batch = 32.0
global_scale = float("$V8_GLOBAL_LR_SCALE")
backbone_ratio = float("$V8_BACKBONE_LR_RATIO")
main_lr = 5e-4 * (train_batch / base_batch) * global_scale
backbone_lr = 2.5e-5 * (train_batch / base_batch) * global_scale * backbone_ratio
...
echo "v8 foundation_detseg_v3 full retrain on new 10671-image dataset"
...
printf '  %-32s %s\n' "DEIMV2_NUM_GPUS" "$DEIMV2_NUM_GPUS"
printf '  %-32s %s\n' "DEIMV2_ALLOW_SINGLE_GPU_FALLBACK" "$DEIMV2_ALLOW_SINGLE_GPU_FALLBACK"
printf '  %-32s %s\n' "V8_GLOBAL_LR_SCALE" "$V8_GLOBAL_LR_SCALE"
printf '  %-32s %s\n' "V8_BACKBONE_LR_RATIO" "$V8_BACKBONE_LR_RATIO"
...
# ---------------------------------------------------------------------------
echo
echo "=== Train DEIMv2 detector on new data (v7 LR) ==="
# ---------------------------------------------------------------------------
export TRAIN_FPATH
export VALI_FPATH
...
for candidate_id in "${CANDIDATES[@]}"; do
    ...
    build_package "$candidate_package_fpath" "$ckpt_fpath" "$SAM2_TRAINED_CKPT" "v8_${candidate_id}"
...
echo
echo "Top v8 checkpoint candidates:"
...
build_package "$FINAL_TUNED_PACKAGE_FPATH" "$SELECTED_CKPT_FPATH" "$SAM2_TRAINED_CKPT" "v8_deimv2_m_sam2_1_hiera_base_plus_tuned"

SELECTED_FINAL_ROOT="$V8_ROOT/final_eval/${SELECTED_CANDIDATE_ID}"
