...
# v7 is the next narrow detector optimization experiment after v6:
...
# - change detector optimization by lowering the global detector LR while
#   preserving the v5 backbone/head ratio
# - select the detector checkpoint using combined raw validation before test
...
V7_ROOT="$EXPT_DPATH/foundation_detseg_v3/v7"
DETECTOR_WORKDIR="$V7_ROOT/train_detector_deimv2_m"
CHECKPOINT_SELECT_DPATH="$V7_ROOT/checkpoint_select"
...
SELECTED_MANIFEST_FPATH="$V7_ROOT/selected_detector_checkpoint.yaml"

PACKAGE_DPATH="$REPO_DPATH/experiments/foundation_detseg_v3/packages"
FINAL_TUNED_PACKAGE_FPATH="$PACKAGE_DPATH/v7_deimv2_m_sam2_1_hiera_base_plus_tuned.yaml"
...
V7_GLOBAL_LR_SCALE="${V7_GLOBAL_LR_SCALE:-0.8}"
V7_BACKBONE_LR_RATIO="${V7_BACKBONE_LR_RATIO:-1.0}"

CANDIDATES=(${V7_CANDIDATES:-checkpoint0019 checkpoint0024 checkpoint0029 checkpoint0034 checkpoint0039 checkpoint0044 checkpoint0049 checkpoint0054 checkpoint0059 best_stg1 last})
...
base_batch = 32.0
global_scale = float("$V7_GLOBAL_LR_SCALE")
backbone_ratio = float("$V7_BACKBONE_LR_RATIO")
...
echo "v7 foundation_detseg_v3 detector-global-lr experiment"
...
printf '  %-32s %s\n' "V7_GLOBAL_LR_SCALE" "$V7_GLOBAL_LR_SCALE"
printf '  %-32s %s\n' "V7_BACKBONE_LR_RATIO" "$V7_BACKBONE_LR_RATIO"
printf '  %-32s %s\n' "DETECTOR_SCORE_THRESH" "$DETECTOR_SCORE_THRESH"
printf '  %-32s %s\n' "DETECTOR_NMS_THRESH" "$DETECTOR_NMS_THRESH"
printf '  %-32s %s\n' "CANDIDATES" "${CANDIDATES[*]}"

echo
echo "=== Train detector with lower global LR ==="
export TRAIN_FPATH
export VALI_FPATH
...
for candidate_id in "${CANDIDATES[@]}"; do
    ...
    build_package "$candidate_package_fpath" "$ckpt_fpath" "$SAM2_TRAINED_CKPT" "v7_${candidate_id}"
...
echo
echo "Top v7 checkpoint candidates:"
...
build_package "$FINAL_TUNED_PACKAGE_FPATH" "$SELECTED_CKPT_FPATH" "$SAM2_TRAINED_CKPT" "v7_deimv2_m_sam2_1_hiera_base_plus_tuned"

SELECTED_FINAL_ROOT="$V7_ROOT/final_eval/${SELECTED_CANDIDATE_ID}"
