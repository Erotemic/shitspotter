...
# Train a small-data DEIMv2 detector and pair it with a frozen tuned SAM2
# checkpoint for fast detector+segmenter comparisons. The default comparison is
# intentionally detector-centric: reuse the strong tuned v5 SAM checkpoint so a
# small-data run mostly measures detector quality and checkpoint choice.
...
RUN_NAME="${RUN_NAME:-deimv2_sam2_small_default}"
...
RUN_DPATH="$(small_data_runs_root)/deimv2_sam2/${SMALL_DATA_COHORT_NAME}/${RUN_NAME}"
...
if [ -z "$SAM2_TRAINED_CKPT" ]; then
    SAM2_TRAINED_CKPT="$EXPT_DPATH/foundation_detseg_v3/v5/train_segmenter_sam2_1_hiera_base_plus/checkpoints/checkpoint.pt"
fi
...
build_package() {
    ...
    "$PYTHON_BIN" -m shitspotter.algo_foundation_v3.cli_package build \
        ...
        --metadata_name "deimv2_sam2_small_$(basename "$package_fpath" .yaml)" >/dev/null
}

evaluate_combined_split() {
    local src_fpath="$1"
...
printf 'DEIMv2+SAM2 small-data run\n'
printf '  %-22s %s\n' "COHORT_DPATH" "$COHORT_DPATH"
printf '  %-22s %s\n' "RUN_DPATH" "$RUN_DPATH"
printf '  %-22s %s\n' "SAM2_TRAINED_CKPT" "$SAM2_TRAINED_CKPT"
...
printf 'candidate_id\tckpt_fpath\tcombined_vali_ap\n' > "$SUMMARY_FPATH"
BEST_AP=""
BEST_CANDIDATE=""
BEST_CKPT=""
for candidate_id in "${CANDIDATES[@]}"; do
    ...
    candidate_eval_dpath="$EVAL_DPATH/${candidate_id}/combined_vali"
    build_package "$candidate_package_fpath" "$ckpt_fpath"
    evaluate_combined_split "$SMALL_DATA_VALI_KWCOCO_FPATH" "$candidate_package_fpath" "$candidate_eval_dpath"
...
evaluate_combined_split "$SMALL_DATA_TEST_KWCOCO_FPATH" "$FINAL_PACKAGE_FPATH" "$RUN_DPATH/test_eval"
TEST_AP="$(read_ap "$RUN_DPATH/test_eval/eval/detect_metrics.json")"

printf 'Selected DEIMv2+SAM2 checkpoint: %s (vali_ap=%s, test_ap=%s)\n' "$BEST_CANDIDATE" "$BEST_AP" "$TEST_AP"
