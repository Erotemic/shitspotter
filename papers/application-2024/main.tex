...
Although this initial search was somewhat ad-hoc, it provided valuable insights into the optimal
...
The scripts used to reproduce these experiments, as well as a log of the ad-hoc experiments, are available
  in the code repository.
Additionally, the trained models are packaged and distributed with information about their training
  configuration.
...
\caption[]{
    (a) Scatterplot illustrating pixelwise Average Precision (AP) and Area Under the Curve (AUC) for the top
      5 checkpoints evaluated on the validation set.
    Points of the same color represent checkpoints from the same training run, which used identical
      hyperparameters.
    (b) Box-and-whisker plot showing the distribution of AP values across the top 5 checkpoints evaluated on
      the validation set.
    The corresponding hyperparameter settings and maximum AP scores for each run are detailed in
...
Next, we rasterized the truth polygons at the same prediction resolution (1/2 the resolution of full
  images).
The corresponding truth and predicted pixels were accumulated into a confusion matrix, allowing us to
  compute standard metrics such as precision, recall, F1, etc.
...
By sweeping a range of thresholds, we calculated the average precision (AP) and the area under the ROC curve
  that plots recall against false positive rate (AUC).
...
We plotted the AP and AUC for the top 5 AP-maximizing results from each of the 7 training runs.
We also created a box-and-whisker plot for these top 5 results, which assigned a color and label to each
  training run.
These plots are shown in \Cref{fig:apauc_scatter}.
The details of the top model for each run, along with relevant hyperparameters for the training run, are
  presented in \Cref{tab:parameters_and_results}.
...
However, while the test AP for this model is good, it is significantly lower at $0.5051$.
...
The test and validation sets show clear responses to objects of interest, but the test set often fails on
  close-up and partially deteriorated poops.
This suggests a bias in the dataset towards "fresh" poops taken from some distance.

Notably, the much larger training set also contains errors, indicating that there is still more information
  that can be extracted from this dataset, perhaps using hard mining techniques.
There are clear difficult cases in certain sticks, leafs, and dark areas on snow.
We note that while compiling these results, we discovered 14 cases where an object failed to be annotated,
  and it is likely that more are missed, but we believe these cases are rare.
...
Both IPFS and BitTorrent are effectively content addressable at the dataset level, which makes them both
  appropriate for our use case where we seek a static address that can be used to robustly access data.
...
{\tt bafybeiedwp2zvmdyb2c2axrcl455x fbv2mgdbhgkc3dile4dftiimwth2y}
...
in addition to the DHT and has a magnet URL of: {\tt 
magnet:?xt=urn:btih:
...
The centralized mechanisms, Girder and Rsync, tend to exhibit faster transfer times compared to the
  decentralized mechanisms, IPFS and BitTorrent for the case where few seeders are available.
}
...
For centralized distribution, we use a self-hosted instance of the Girder data management platform
  \cite{girder_2024}.
For decentralized distribution, we utilize the Transmission \cite{transmission_2024} client for BitTorrent
  and Kubo \cite{ipfskubo_2024} for IPFS.
As a baseline method, we also include the time of direct transfers using Rsync\cite{rsyncprojectrsync_2024}.
...
The ZeroWaste dataset \cite{bashkirova_zerowaste_2022} (9.9GB) contains 1,800 segmented video frames and
...
The TACO dataset \cite{proenca_taco_2020} (17GB) is another "living" dataset containing 1,500 images with
