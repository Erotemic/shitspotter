...
\caption[]{
    The pixelwise AP and AUC of the top-5 checkpoints evaluated on the validation set.
...
\caption{
    The range of AP values over the top-5 checkpoints evaluated on the validation set.
...
This restricted set is illustrated in \Cref{tab:parameters_and_results}.
...
% See: ./scripts/gather_heatmap_examples.py
%\begin{figure*}[h]
%\centering
%\includegraphics[width=1.0\textwidth]{figures/test_heatmaps_with_best_test_model.jpg}
%\caption[]{
%    Qualitative results on a representative test set sample using the highest
%    scoring model on the test set.
%    Success cases are on the left, and failures increase towards the right.
%    The confusion 

%    The top rows shows true positive pixels in white, false positive pixels in
%    red, false negative pixels in teal, and true negative pixels in black.  For
%    visualization purposes, the threshold to binarize the prediction is chosen
%    to maximize the F1 for each image, thus the top row is in some sense the
%    best possible classification of the heatmap, which is shown in the middle
%    row. The final row is the input image.
...
%}
%\label{fig:test_heatmaps_with_best_test_model}
%\end{figure*}
...
%\begin{figure*}[h]
%    % /home/joncrall/data/dvc-repos/shitspotter_expt_dvc/_shitspotter_evals/eval/flat/heatmap_eval/heatmap_eval_id_6b952f74/heatmaps/_loose_images/
%\centering
%\caption[]{
%    Qualitative results on select validation images using the highest scoring
...
%\centering
%\caption[]{
%    Qualitative results on select train images using the highest scoring model
%    on the validation set.

%    Cases on the left are easier and get progressively harder
...
Qualitative results for the test, validation, and training set are shown in \cref{fig:test_heatmaps_with_best_vali_model}. 
...
method        &       &        &        &       &        \\
\midrule
bittorrent    &     2 &  33.32 &  37.41 &  6.87 &  59.77 \\
girder-upload-GUI &     1 &   5.88 &    nan &  5.88 &   5.88 \\
ipfs          &     2 &   3.22 &   2.02 &  1.80 &   4.65 \\
rsync         &     1 &   6.10 &    nan &  6.10 &   6.10 \\
rsync-local   &     1 &   0.01 &    nan &  0.01 &   0.01 \\
\bottomrule
\end{tabular}
\caption[]{
