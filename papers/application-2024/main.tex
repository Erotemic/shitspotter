...
The best model achieves a pixelwise average precision of 0.83 on a 691-image validation set and 0.51 on a
  small independently captured 30-image contributor test set.
...
Model weights are made publicly available with the dataset. Experimental
...
\caption[]{
    ...
    %This is 
    %This also illustrates the typical case where the poop is small relative to the image size.
...
\toprule
Name & Categories & Images & Annotations & Image W \times H & Annot Area$^{0.5}$ & Size & Annot Type \\
\midrule
...
visualizes the annotations of all datasets.
...
\caption[]{
    Oriented bounding box size distributions of each dataset.
}
...
\begin{figure*}[ht]
\centering
\includegraphics[width=1.0\textwidth]{plots/appendix/dataset_compare/combo_polygon_area_vs_num_verts_jointplot.png.png}
\caption[]{
    Polygon area versus number of vertices for each dataset.
}
