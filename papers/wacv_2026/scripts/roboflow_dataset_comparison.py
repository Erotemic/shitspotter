"""
Modified version of ~/code/shitspotter/papers/wacv_2026/scripts/related_dataset_comparison.py
to inspect the roboflow datasets.
"""
datasets = []
datasets.extend([
    {
        'name': r'Roboflow/han-bjamu/poppopo/2/',
        'coco_fpath': '/data/joncrall/dvc-repos/roboflow-repos/han-bjamu/poppopo/2/data_imgs490_4d00165b.kwcoco.zip',
    },
    {
        'name': r'Roboflow/pazini/dog-poop-detection-uip1h/3',
        'coco_fpath': '/data/joncrall/dvc-repos/roboflow-repos/pazini/dog-poop-detection-uip1h/3/data_imgs134_48d426f0.kwcoco.zip',
    },
    {
        'name': r'Roboflow/new-workspace-j42am/softpoop/1',
        'coco_fpath': '/data/joncrall/dvc-repos/roboflow-repos/new-workspace-j42am/softpoop/1/data_imgs319_effb90b5.kwcoco.zip',
    },
    {
        'name': r'Roboflow/new-workspace-j42am/dp-4iarz/1',
        'coco_fpath': '/data/joncrall/dvc-repos/roboflow-repos/new-workspace-j42am/dp-4iarz/1/data_imgs74_0555d869.kwcoco.zip',
    },
    ...
    # {
    #     'name': '``ScatSpotter\'\'',
    #     'coco_fpath': '/home/joncrall/code/shitspotter/shitspotter_dvc/full-poop-only.kwcoco.zip',
    # },
])
...
def main():
    ...
    if 1:
        ...
        for dset in ub.ProgIter(dsets, verbose=3, freq=1):
            ...
            stamp = ub.CacheStamp('plots', depends=hashid, dpath=cache_dpath, verbose=3)
            # stamp.clear()
            dpath = bundle_dpath / ('_visual_stats_' + name + hashid[0:8])
            scalar_stats = fpath_to_stats[dset.fpath]
            if stamp.expired() or not dpath.exists():
    ...
    if True:
        ...
        for name, paths in accum.items():
            ...
            dpath = ub.Path('/home/joncrall/code/shitspotter/papers/wacv_2026/plots/appendix/dataset_compare_roboflow')
            dpath.ensuredir()
            ...
            print(f'write: canvas_fpath={canvas_fpath}')
            dpath.ensuredir()
