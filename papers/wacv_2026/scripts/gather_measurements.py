...
def main():
    ...
    is_null = subtable2['duration'].isna()
    is_null_loc = is_null.index[is_null]
    _df = subtable2.loc[is_null_loc]
    subtable2.loc[is_null_loc, 'duration'] = _df['time.end_date'] - _df['time.start_date']

    rich.print(subtable2.to_string())

    subtable3 = subtable2[subtable2['status'] != 'outlier']
    subtable3 = subtable3[subtable3['status'] != 'failed']
    # subtable3 = subtable3[subtable3['status'] == 'success']
    groups = subtable3.groupby(['method', 'action'])
