...
%While not traditional for dataset papers, data accessibility, province, and reproducibility
%  are a critical component that we believe deserves more attention.
...
\begin{table}[hb]
\centering
\setlength{\tabcolsep}{3pt} % Adjusted padding for new column
...
Transfer times (in hours) for our 42GB dataset: trials (n), mean ($\mu$), std ($\sigma$).
Each experiment was run 5 times.
Uncompressed transfers provide granular access to individual files, while compressed (zipped) transfers are faster.
}
\label{tab:transfertime}
\end{table}
